...
- ✅ **Editable Use-Case + Customization boxes in deck Custom mode** (2026-02 · current) — Slide 5 (Use Cases, 6 boxes) and Slide 7 (Customization, 4 boxes) were previously only editable for title/italic-line copy; the boxes themselves were hardcoded from `content.js`. Added a generic `type: "list"` field to the deck manifest with a per-item shape (`itemFields`) plus a `defaults` reference. New `ListField` editor in SlideEditor renders each item as an editable card with Title + Body inputs, **Add / Delete / Move-up / Move-down** controls per row, and a **Reset to defaults** action that un-overrides the list. When no override is set, the editor seeds the UI with the published defaults so admins can tweak rather than start from scratch. BlackRock.jsx slide 5 and slide 7 prefer `ov(slide, "items", null)` when present, else fall back to `CORPORATE_USE_CASES` / `CUSTOMIZATION`. Empty array (`[]`) is an explicit "show no boxes" signal. End-to-end verified: edit + delete + add + reorder operations all persist and render correctly on the public `/deck/<slug>` URL. **92/92 pytest still pass.** Files: `/app/frontend/src/admin/deckManifest.js` (list-field type, defaults wired to content.js), `/app/frontend/src/admin/SlideEditor.jsx` (`ListField` component), `/app/frontend/src/pages/BlackRock.jsx` (slide 5 + slide 7 override-aware).
