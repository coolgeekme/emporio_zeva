...
- ✅ **Editable Use-Case + Customization + Pricing-tier boxes in deck Custom mode** (2026-02 · current) — Slides 5 (Use Cases, 6 boxes), 7 (Customization, 4 boxes), and 8 (Pricing Tiers, previously fixed at 2) were previously locked or only partially editable. Added a generic `type: "list"` field to the deck manifest with a per-item shape (`itemFields`) plus a `defaults` reference. New `ListField` editor in SlideEditor renders each item as an editable card with per-field inputs, **Add / Delete / Move-up / Move-down** controls per row, and a **Reset to defaults** action that un-overrides the list. When no override is set, the editor seeds the UI with the published defaults so admins can tweak rather than start from scratch. BlackRock.jsx slides 5, 7, 8 prefer `ov(slide, "items", null)` when present, else fall back to `CORPORATE_USE_CASES` / `CUSTOMIZATION` / `CORPORATE_PACKAGES`. **Slide 8 also got a UX upgrade:** the dark "hero" card treatment now follows any tier that has a `badge` (instead of being hardcoded to position 2), and the grid auto-adapts to tier count (1→single column, 2→two cols, 3+→three cols). The `includes` field accepts either an array (defaults) or newline-separated string (editor) — render normalizes. Legacy `package_1_*` / `package_2_*` named fields removed from manifest (no decks in DB had overrides, so no migration needed). End-to-end verified on `/deck/<slug>`: edit + delete + add + reorder all persist for slides 5, 7, and 8 — confirmed by adding a $128 "Founders" 3rd tier from scratch. **92/92 pytest still pass.** Files: `/app/frontend/src/admin/deckManifest.js` (list field type, three new list fields, `PRICING_LIST_DEFAULTS` transformer), `/app/frontend/src/admin/SlideEditor.jsx` (`ListField` component), `/app/frontend/src/pages/BlackRock.jsx` (slides 5/7/8 override-aware + adaptive grid + badge-driven hero styling).
