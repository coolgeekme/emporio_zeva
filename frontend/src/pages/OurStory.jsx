...
export default function OurStory() {
  ...
  const r2 = useReveal();
  const r3 = useReveal();
  const c = useSiteContent("our_story");
    ...
    <div className="pt-[90px]" data-testid="our-story-page">
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-10 pt-20 md:pt-28 pb-20 md:pb-28 grid grid-cols-1 md:grid-cols-12 gap-10 items-end">
        <div className="md:col-span-7">
          <p className="overline text-[#C05A3A]">{c("hero_overline", TAGLINES.italian_tradition)}</p>
          <h1 className="font-serif text-5xl md:text-6xl lg:text-[88px] leading-[1.02] tracking-tight mt-6 text-[#2A1F1D]">
            {c("hero_h1_line1", "A Sicilian")}
            <br />
            <span className="italic text-[#C05A3A]">{c("hero_h1_line2", "tradition.")}</span>
          </h1>
        </div>
        <div className="md:col-span-5 md:pl-10">
          <p className="text-[#5C4E4A] leading-relaxed">
            {c("hero_body", "Returning to my roots, I reconnected with my grandmother's recipe and the chocolate tradition of Modica, Sicily — brought together in a confection designed to surprise, to be sliced and shared.")}
          </p>
        </div>
      </section>
      ...
      <section className="reveal" ref={r1}>
        <div className="max-w-[1400px] mx-auto px-6 md:px-10">
          <div className="img-wash aspect-[16/9] md:aspect-[21/9]">
            <img src={c("founder_image", IMAGES.founder)} alt="Eva, founder of Emporio Zeva" data-testid="story-founder-image" />
          </div>
          <p className="overline text-[#5C4E4A] mt-4">
            {c("founder_caption", `${BRAND.founder} · Founder · From Sicily, with seriousness`)}
          </p>
        </div>
      </section>
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-10 grid grid-cols-1 md:grid-cols-12 gap-8 reveal" ref={r3}>
        <div className="md:col-span-7 img-wash aspect-[4/3]">
          <img src={c("sicily_image", IMAGES.sicily)} alt="Modica, Sicily — view of the town" data-testid="story-sicily-image" />
        </div>
        <div className="md:col-span-5 img-wash aspect-[4/5] md:translate-y-16">
          <img src={c("italian_moment_image", IMAGES.italian_moment)} alt="Italian moment, espresso ritual" data-testid="story-italian-moment-image" />
        </div>
      </section>
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-10 py-32 grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
        <div className="md:col-span-8 md:col-start-3 text-center">
          <p className="overline text-[#C05A3A]">{c("closing_overline", "A modern ritual")}</p>
          <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl leading-[1.05] mt-5 text-[#2A1F1D]">
            {c("closing_h2_line1", "Everything is produced in small batches in California")}
            <br />
            <span className="italic text-[#C05A3A]">{c("closing_h2_line2", "with a hands-on approach.")}</span>
          </h2>
          <p className="mt-8 text-[#5C4E4A] leading-relaxed max-w-2xl mx-auto">
            {c("closing_body", "Italian tradition, crafted in California. Unexpected in appearance yet deeply nostalgic at heart — meant to create a moment of surprise, sharing, and conversation.")}
          </p>
